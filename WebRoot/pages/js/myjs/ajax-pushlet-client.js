...
var PL = {
	...
	STATE_LISTENING: 3,
	state: 1,
	onEvent : function(event){//事件获取,实现在index.js中
...
/** Join, listen and subscribe. */
	joinListen: function(aSubject, sessionId, companyId) {
		PL._setStatus('join-listen ' + aSubject);
		// PL.join();
		// PL.listen(aSubject);
		// 初始化sessionId和companyId
...
// Do XML HTTP request
	_doRequest: function(anEvent, aQuery) {
		...
		var url = GLOBAL.baseUrl+'/pushlet.srv?p_event='+anEvent;
		...
		if (PL.sessionId != null && PL.companyId != null) {
			url = url + '&p_id=' + PL.sessionId + '&p_companyId=' + PL.companyId;
			if (anEvent == 'p_leave') {
				PL.sessionId = null;
			}
		}
	...
	_getJSON: function(urlStr, cb) {
		...
		$.jsonp({
			...
			success: function(data) {
				var rows = data.rows;
				PL.debug('_getJSON eventCnt=', rows.length);
				// 构建事件数组
				var events = new Array(rows.length);
				for (i = 0; i < rows.length; i++) {
					events[i] = new PushletEvent(rows[i]);
				} 
...
/** Handle incoming events from server. */
	_onEvent: function (event) {
		...
		if (eventType == 'data') {
			...
			// 刷新事件到来，发出类型为refresh的刷新信号，服务端依照此时间间隔返回消息。
			if (PL.state < PL.STATE_LISTENING) {
